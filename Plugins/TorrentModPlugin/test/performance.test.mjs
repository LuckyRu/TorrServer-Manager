...
const BUDGETS = {
    ...
    // Разбор заголовка стоит ~60 % всего intake, поэтому дубли отбрасываются до него: 1080
    // различных записей вместо 2160 разборов, как было до дедупликации.
    parseCalls: 1550,
    ...
    targetTitleReads: 1380,
    // console в горячем пути: на WebOS вызов с объектом ещё и удерживает граф от сборки мусора.
    consoleCalls: 25,
    // Трекер доставляет дельту: получатель прогоняет каждый доставленный элемент через слияние
    // по всему пулу, поэтому переотправка накопленного делала стоимость квадратичной.
    deliveredItemsVolume: 800,

    // --- домен: 9 ревизий пула, 24 серии ---
    ...
    selectionReadsPerItemEpisode: 3.6,
