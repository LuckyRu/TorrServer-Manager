...
Статус: реализация первой очереди. Документ описывает целевую архитектуру, порядок миграции и критерии
готовности. Уже реализованы кэш производных проекций, кадровый scheduler, дифф-обновление строк,
очистка закрытого picker и ограниченная коллекция штатного `Navigator` Lampa. Оставшиеся этапы отмечены как следующие
очереди. Документ не предлагает переписывать экран на новый UI-фреймворк и не меняет поисковую семантику,
скоринг или навигационный контракт Lampa.
...
- [`search/scoring.js`](../../Plugins/TorrentModPlugin/search/scoring.js) фильтрует, оценивает и сортирует
