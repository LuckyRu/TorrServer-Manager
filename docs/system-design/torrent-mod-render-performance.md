...
**Узкое место после этих шагов сместилось и больше не находилось в рендере**, а в разборе ответов
трекеров. Вторая волна работ это закрыла: сквозной поиск подешевел на 37 %, а наибольший
неразрываемый кусок main thread — вдвое. Разбор, замеры и перечень правок — в разделе
[«Вторая волна: intake, а не рендер»](#вторая-волна-intake-а-не-рендер). Регрессии на всё
перечисленное ловит [`test/performance.test.mjs`](../../Plugins/TorrentModPlugin/test/performance.test.mjs)
в общем прогоне `npm run test:plugin`.
...
### Результат второй волны

Замер одним и тем же измерителем на исходном коде (`git worktree` на master) и на итоговом,
сценарий теста: 9 трекеров, 2 запроса плана, 1080 различных сырых записей, пул 720 раздач.

| Показатель | До | После | |
|---|---:|---:|---:|
| Сквозное CPU поиска | 124 мс | 78 мс | −37 % |
| Наибольший неразрываемый блок | 3.9 мс | 2.2 мс | −44 % |
| То же, оценка на WebOS (×10) | ~39 мс | ~22 мс | |
| Разборов заголовка | 2160 | 1440 | −33 % |
| Пересчётов названий цели | 2931 | 1253 | −57 % |
| Доставлено элементов | 1320 | 720 | −45 % |
| Чтений метаданных на «серия × раздача» | 6.21 | 3.21 | −48 % |
| Объектов в консоль на проекцию пикера | 1 | 0 | |
| Синхронных блоков | 18 | 54 | больше и короче |

Число блоков выросло намеренно: та же работа теперь нарезана на втрое большее количество втрое
более коротких кусков. Именно это, а не суммарное CPU, определяет отзывчивость.

### Что сделано, по убыванию отдачи

1. **`runSearchGates` был одной неразрываемой задачей.** `deferWork` уводил разбор в macrotask, но
   целиком: parse + два гейта + `compileReleaseSelection` для всего ответа трекера за один
   синхронный проход. Проход режется потолком в 32 записи и бюджетом в 8 мс: потолок даёт точку
   выхода на любом устройстве, бюджет укорачивает срез там, где устройство медленнее ожидаемого.
   Непрогрессивный путь переведён на последовательную цепочку промисов — `Promise.all` свёл бы
   чанкование на нет. **Этот пункт даёт отзывчивость сам по себе, независимо от остальных.**

2. **Дедупликация стояла после разбора.** Трекер отвечает на каждый запрос плана отдельно, каждый
   ответ проходил полный разбор, и только потом `mergeItems` схлопывал дубли по `releaseIdentity`.
   `buildQueries` даёт до 4 запросов, `buildAnimeQueries` — до 12, а выдачи по «X S01» и
   «X 1 сезон» пересекаются почти полностью. Ключ `Title|Size` считается до разбора и живёт весь
   поиск в состоянии трекера. Побочный эффект: воронка теперь считает различные раздачи — повтор
   одного заголовка по разным запросам это один релиз, а не две находки.

3. **`titleAnalysis` считался дважды на каждую принятую раздачу.** Гейт названия считал анализ и
   выбрасывал его, а `compileReleaseSelection` вызывался с `analysis === undefined` и считал всё
   заново. Теперь `evaluateSearchTitleGate` отдаёт анализ в `details` успешного решения — в
   диагностические сводки он не утекает, потому что те собирают `details` только отказов.

4. **`normalizedTitleKey` без мемоизации.** `.normalize('NFKC')` и Unicode-regex на каждый вызов при
   ~50 вызовах на разбор одного заголовка и повторяющихся аргументах: названия и алиасы цели одни и
   те же для всей выдачи. Кэш ограничен 2000 записями и сбрасывается целиком — приложение живёт на
   телевизоре неделями, а вытеснение по одному здесь не окупает сложности.

5. **Трекер переотправлял всё накопленное.** Каждое обновление слало `state.items.slice()`, а
   получатель прогонял каждый доставленный элемент через слияние по всему пулу — на девяти трекерах
   стоимость выходила квадратичной. Теперь уходит дельта: добавленные раздачи и вытеснившие их
   варианты, сравнение по ссылке без повторного расчёта `releaseIdentity`. Накопленный размер
   отдаётся отдельным числом `totalItems` для диагностики.

6. **Логирование в горячем пути.** `selectPickerData` собирал списки кандидатов и отказов ради одной
   строки в console при **каждом** пересчёте проекции — дороже самой проекции. Осталась строка,
   структурный снимок ушёл в verbose. Сводка гейтов собирала до 300 объектов с заголовками отказов
   на каждый ответ трекера независимо от того, читает их кто-нибудь: счётчики причин дёшевы и нужны
   экрану всегда, детали теперь только в диагностическом буфере. `debugEnabled()` читается один раз
   на ответ трекера вместо каждой раздачи, а `diagnosticsState()` перестал пересоздавать два
   замыкания на каждый вызов.

7. **CSS анимировал layout ровно во время burst'а.** Чипы трекеров анимировали `max-width`,
   `padding` и `margin`: 9 чипов × (появление + исчезновение) × 250 мс пересчёта раскладки строки
   чипов каждый кадр, и именно тогда, когда main thread занят разбором. Шиммер бейджей ехал
   `background-position` — не композитится, поэтому все строки сезона перекрашивались каждый кадр
   всё время холодного поиска. Теперь `opacity`/`transform` и блик на псевдоэлементе;
   `contain: layout` на строках и элементах пикера — без `paint`, потому что иконка выходит за верх
   строки, а фокус рисуется `box-shadow`, и paint-containment обрезал бы оба.
   **Компромисс:** уходящий чип больше не схлопывает ширину, а держит место до удаления из DOM —
   схлопывание ширины без анимации layout невозможно.

8. **Дублирующая работа в цикле «серия × раздача».** `evaluateIdentityGate` вызывался дважды подряд
   с теми же аргументами (второй раз внутри `passesMatchGate`) — для 720 раздач на 24 серии это
   17 тысяч лишних вычислений покрытия на ревизию пула. `releaseIdentity` считался на каждую пару
   ради сравнения с сохранённым выбором — теперь один раз на раздачу. Заголовки отсеянного
   собирались и на пути бейджей, который их выбрасывает. `updateEpisodeBadges` искал узлы в DOM до
   проверки сигнатуры, то есть обходил дерево по каждой строке даже на неизменившемся кадре.

### Что осознанно не сделано

- **Сигнатура фильтров через `JSON.stringify`** (`syncFilterChips`). Была в списке, но замер её не
  оправдал: вызывается не чаще раза на ревизию пула, порядка 20 мкс. Замена на сравнение по ссылке
  сделала бы хуже — проекция мемоизируется по `poolRevision`, поэтому новый объект приходил бы на
  каждую ревизию и заставлял бы `Lampa.Filter` перестраивать меню там, где сейчас ничего не делается.
- **Пропуск `Lampa.Layer.update()` для кадров, где менялись только бейджи.** Бюджет «не более одного
  на кадр» и так выполняется, а высота элементов — задокументированная ловушка проекта: закэшированная
  в `scroll.minus()` математика протухает молча и проявляется только на телевизоре. Экономия одного
  forced layout на кадр не стоит этого класса регрессий.
- **Web Worker.** Причиной фризов был размер синхронного куска, а не объём работы; после пункта 1
  worker добавил бы сериализацию и протокол отмены, не решая ничего.

Осталось: замер на телевизоре по бюджетам из шага 7 — оценка ×10 к desktop V8 пока не подтверждена
живым профилем.
...
|---|---:|---:|---|
| `parseCalls` | 1440 | 1550 | дедупликацию до разбора (пункт 2) |
| `maxParseBurst` | 32 | 36 | размер синхронного куска (пункт 1) |
| `targetTitleReads` | 1253 | 1380 | переиспользование анализа заголовка (пункты 3–4) |
| `consoleCalls` | 21 | 25 | логирование в горячем пути (пункт 6) |
| `deliveredItemsVolume` | 720 | 800 | доставку дельты вместо накопленного (пункт 5) |
| `consoleObjectsPerPickerProjection` | 0 | 0 | сборку списков ради console (пункт 6) |
| `projectionRunsPerBurst` | 9 | 9 | мемоизацию проекций (шаг 2) |
| `selectionReadsPerItemEpisode` | 3.21 | 3.6 | константу цикла «серия × раздача» (пункт 8) |
...
Бюджеты ужаты по факту после второй волны и проверены как храповик: тот же тест, запущенный против
исходного кода в `git worktree` на master, даёт 6 падений из 11. Откат любой из оптимизаций
не пройдёт молча.
...
| снятый потолок среза | `maxParseBurst` | 32 → 120 ❌ |
| возвращённая сборка списков в `log` пикера | `consoleObjectsPerPickerProjection` | 0 → 1 ❌ |
