...
|---|---|---|
| A — правки кода upstream, в начале стопки | по коммиту на **судьбу изменения**: что отдать в upstream (`ReadHeaderTimeout`; ядро торрента под нескольких зрителей), что перенести в менеджер (умолчания настроек), что вынести из чужих файлов (описание форка в `README`) | единственное место, где возможны конфликты |
