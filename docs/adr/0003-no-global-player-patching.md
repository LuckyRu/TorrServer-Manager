...
Между этой моделью и текущей была ещё одна: ffprobe-гейт кодеков заменили на нативный
`url_reserve`-фолбек плеера (прямой стрим основным, GST — резервным при фатальной ошибке
декодирования) — решение и отвергнутые альтернативы (в т.ч. безусловный GST) задокументированы в
[ADR-0006](0006-native-player-fallback-not-ffprobe-gate.md). Актуализация этого ADR от 2026-08-09
заменила и её: GST стал основным транспортом для всех файлов, `url_reserve` не используется.
