...
- **GStreamer: организация пайплайнов по форматам** (почему решение «как играть файл»
  переоткрывается в восьми местах и как это даёт расхождения; правило «не ремуксируем —
  транскодируем» с живой проверкой на MPEG-2 в MKV; целевая модель Capabilities → Plan →
  реализация и порядок перехода) →
